...
public class JdbcTokenRepositoryImpl extends JdbcDaoSupport implements
    ...
    public JdbcTokenRepositoryImpl(String prefix) {
        this.prefix = prefix;
        this.tokensBySeriesSql = "select username,series,token,last_used from "+prefix+"persistent_logins where series = ?";

//        CREATE_TABLE_SQL =
        this.insertTokenSql = "insert into " + prefix + "persistent_logins (username, series, token, last_used) values(?,?,?,?)";
        this.updateTokenSql = "update " + prefix + "persistent_logins set token = ?, last_used = ? where series = ?";
        this.removeUserTokensSql = "delete from " + prefix + "persistent_logins where username = ?";
    }
...
//        if (createTableOnStartup) {
        try {
            String sql = "create table " + prefix + "persistent_logins (username varchar(64) not null, series varchar(64) primary key,\n" +
                    "token varchar(64) not null, last_used date not null)";
